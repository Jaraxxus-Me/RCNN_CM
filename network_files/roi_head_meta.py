...
def fastrcnn_loss(class_logits, box_regression, labels, meta_label, regression_targets, phase):
    ...
    # calculate cls loss
    pos_ind = torch.where(torch.gt(map_label, 0))[0]
    neg_ind = torch.where(torch.eq(map_label, 0))[0]
    ...
    classification_loss = 0.25*classification_loss_pos+classification_loss_neg
